...
// Public read-only endpoints (GET only). Everything else under /api/admin stays protected.
const PUBLIC_ADMIN_READ_PATHS = [
  "/api/admin/pricing",
...
export async function middleware(request: NextRequest) {
  ...
  const isPublicAdminRead =
    request.method === "GET" && PUBLIC_ADMIN_READ_PATHS.includes(pathname);
  const isAdminApi =
