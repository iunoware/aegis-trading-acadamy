...
function mapAdminToProfileRecord(admin: AdminUser): ProfileRecord {
  const name =
    admin.name || [admin.firstName, admin.lastName].filter(Boolean).join(" ") || "Admin";
...
export default function ProfilePageClient({
  ...
  const handleSaveProfile = async (request: ProfileUpdateRequest) => {
    ...
    try {
      ...
      // Only the password section is actually wired to the backend — the
      // name/email/phone fields are rendered read-only in ProfileForm right
      // now, so there's nothing on the server to persist for those yet.
      if (password) {
        await axios.patch(
      ...
      // Bumping updatedAt changes the `key` on <ProfileForm>, which remounts
      // it and clears the password fields back to empty.
      setProfile((current) =>
        current
          ? {
              ...current,
      ...
      toast.success(
        password ? "Password updated successfully" : "Profile updated successfully",
        {
          description: password
