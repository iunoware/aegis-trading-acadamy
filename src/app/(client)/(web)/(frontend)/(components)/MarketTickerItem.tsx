...
export const MarketTickerItem: React.FC<MarketTickerItemProps> = React.memo(
  function MarketTickerItem({ item }) {
    ...
    return (
      <div className="inline-flex items-center gap-2.5 text-xs sm:text-sm  font-mono whitespace-nowrap px-1">
        {/* Optional Icon / Badge */}
        {icon && <span className="text-sm">{icon}</span>}

        {/* Symbol */}
        <span className="text-white font-bold font-sans tracking-wide">{symbol}</span>
        ...
        <span
          className="text-[var(--primary)]/50 font-bold ml-6 sm:ml-8"
          aria-hidden="true"
        >
          •
        </span>
