...
//       const ext = path.extname(newFile.name) || ".mp4";
//       const fileName = `${crypto.randomUUID()}${ext}`;
//       const newInternalRef = `${courseId}/${fileName}`;

//       const nodeStream = Readable.fromWeb(
//         newFile.stream() as import("stream/web").ReadableStream<Uint8Array>,
//       );
...
//         await upload.done();
//       } catch (uploadError) {
//         console.error("S3 upload failed while replacing video:", uploadError);
//         return NextResponse.json(
//           { success: false, message: "Failed to upload replacement video." },
//           { status: 500 },
//         );
//       }

//       // Only delete the old object after the new one is confirmed uploaded,
//       // so a failed replacement never leaves the lesson with no video at all.
//       if (!existing.videoUrl.startsWith("http")) {
//         await deleteVideoFile(existing.videoUrl).catch((err) =>
//           console.error(
//             `Failed to delete replaced video file for lesson ${lessonId}:`,
...
import { ActivityAction, ActivityActorType } from "@/generated/prisma/client";
import { getRequiredSuperAdmin } from "@/lib/current-user";
import { saveVideo, deleteVideoFile } from "@/lib/video-storage";
...
export async function PATCH(
  ...
  try {
    ...
    if (newFile instanceof File) {
      ...
      let newInternalRef: string;
      try {
        newInternalRef = await saveVideo(newFile, courseId);
      } catch (saveError) {
        console.error("Local video save failed while replacing video:", saveError);
        return NextResponse.json(
          { success: false, message: "Failed to save replacement video." },
          { status: 500 },
        );
      }
